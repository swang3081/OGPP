...
Training script for the Thomson problem, supporting multiple modes:

1. 2D mode (--mode 2d):
   - Data format: (theta, phi, r) -> x1: (theta, phi, nx, ny)
   - x0: uniform random in [-1,1]^2
   - Uses QuadraticHermiteNormalConditionalProbabilityPath
   - r information is encoded in the velocity magnitude
...
3. 3D MiniBatch OT mode (--mode 3d_minibatch_ot):
   - Same data format as 3D mode
   - x0-x1 pairing uses batch-level OT matching

4. 3D EqOTFM mode (--mode 3d_eqotfm):
   - Same data format as 3D mode
...
from flow_lab.paths import QuadraticHermiteNormalConditionalProbabilityPath, LinearConditionalProbabilityPath
from flow_lab.models import UncondUniGBNTransformer, UncondUniGBNTransformer_PE
from flow_lab.utils import save_checkpoint
from flow_lab.datasets import SphereCoordThomsonDataset, XYZDataset, XYZMiniBatchOTDataset, XYZEqOTFMDataset
...
def build_argparser():
    ...
    p.add_argument("--mode", type=str, default="2d",
                   choices=["2d", "3d", "3d_minibatch_ot", "3d_eqotfm"],
                   help="Training mode: 2d, 3d, 3d_minibatch_ot, or 3d_eqotfm")
    ...
    # Logging parameters
    p.add_argument("--exp_name", type=str, default="thomson_2d")
...
def vis_begin_3d_sphere(args, dataset, path, device):
    """
    3D sphere visualization

    Plot how the trajectory from t=0 to t=1 evolves on the sphere.
    r = ||velocity|| is used as the radius, with different r values shown in different colors.
    """
    ...
    x0 = torch.from_numpy(x0_np).to(device)  # (k, N, 2)
    x1 = torch.from_numpy(x1_np).to(device)  # (k, N, 4)
    ...
    for tidx, t in enumerate(ts):
        ...
        xt = path.sample_conditional_path_inputx0(x0, x1, tt)  # (k, N, 2)

        # Get the velocity field at the current time; the velocity magnitude is r
        vt = path.conditional_vector_field_inputx0(x0, x1, tt)  # (k, N, 2)
        r_t = torch.linalg.norm(vt, dim=-1)  # (k, N)
        ...
        for i in range(k):
            ...
            pts_2d = xt[i].cpu().numpy()  # (N, 2)
            r_i = r_t[i].cpu().numpy()     # (N,)
            xyz = theta_phi_to_cartesian(pts_2d, r_i)  # (N, 3)
        ...
        if args.output_begin_trajectory:
            # Save (theta, phi, r)
            out = np.concatenate([
                xt[..., :2].detach().cpu().numpy(),
                r_t[..., None].detach().cpu().numpy()
            ], axis=-1).astype(np.float32)  # (k, N, 3)
    ...
    print(f"[VIS] Saved 3D sphere visualizations to: {begin_dir}")
...
def main(args=None):
    ...
    if args.in_out_dim is None:
        # All 3D modes use in_out_dim=3
        args.in_out_dim = 3 if args.mode.startswith("3d") else 2
    ...
    # Select path and dataset based on mode
    if args.mode.startswith("3d"):
        ...
        # 2D mode: use QuadraticHermiteNormalConditionalProbabilityPath
        # normalize_tangent=False: keep the magnitude information of n1 (r) without normalizing
        path = QuadraticHermiteNormalConditionalProbabilityPath(
            p_simple=p_simple,
            p_data=p_simple,
            in_out_dim=args.in_out_dim,
            normalize_tangent=False,  # do not normalize, preserve r information
        ).to(device)

        if _is_rank0():
            print("[Path] Using QuadraticHermiteNormalConditionalProbabilityPath (2D, normalize_tangent=False)")

        # Build the dataset ((theta, phi, r) format)
        dataset = SphereCoordThomsonDataset(
            data_path=args.data_path,
            num_points=args.n_points,
            num_workers=args.num_workers,
            hilbert_p=args.hilbert_p,
            presorted=args.presorted,
        )

        if _is_rank0():
            print(f"[Dataset] Total samples: {dataset.num_meshes}")
            print(f"[Dataset] Presorted: {args.presorted}")

        # 2D mode needs cond_vec_use_x0
        cond_vec_use_x0 = True
    ...
    if args.vis_begin and _is_rank0():
        if args.mode.startswith("3d"):
            ...
            vis_begin_3d_sphere(args, dataset, path, device)
    ...
    trainer = MeshUncondTrainer(
        ...
        cond_vec_use_x0=cond_vec_use_x0,   # needed in 2D mode, not needed in 3D mode
    ...
    try:
        ...
        if _is_rank0():
            ...
            ckpt_path = save_checkpoint(
                ...
                args,
                ckpt_dir,
                arch_name=f"Thomson2D_epoch{trainer.start_epoch+1:03d}",
            )
