...
# -*- coding: utf-8 -*-
"""
run_minimalsurface_multimode.py

Minimal Surface Conditional Generation with multiple training modes:
- default: Standard flow matching (no OT)
- minibatch_ot: MiniBatch OT for batch-level sample matching
- eqotfm: Equivariant OT Flow Matching (point-level + batch-level OT)

Uses anchors (3 anchors) as condition, generates edge_points (256 points)
- Data format: anchors (B, 3, 2) -> flattened to (B, 6) as condition
- Target: edge_points (B, 256, 2) 2D edge points
- Supports CFG (Classifier-Free Guidance) training
"""
...
from flow_lab.datasets import (
    MinimalSurfaceDataset,
    MinimalSurfaceMiniBatchOTDataset,
    MinimalSurfaceEqOTFMDataset,
)
from flow_lab.trainers import MinimalSurfaceCondTrainer
...
def build_argparser():
    p = argparse.ArgumentParser(description="Minimal Surface Conditional Generation (Multimode)")
    ...
    p.add_argument("--mode", type=str, default="default",
                   choices=["default", "minibatch_ot", "eqotfm"],
                   help="Training mode: default, minibatch_ot, or eqotfm")

    # OT-related arguments
    p.add_argument("--ot_solver", type=str, default="hungarian",
                   choices=["hungarian", "greedy"],
                   help="OT solver for batch-level matching")
    p.add_argument("--point_ot_solver", type=str, default="greedy",
                   choices=["hungarian", "greedy"],
                   help="OT solver for point-level matching in EqOTFM mode")
    p.add_argument("--num_workers", type=int, default=30,
                   help="Number of workers for parallel OT computation")

    # Data
    p.add_argument("--data_path", type=str, required=True,
                   help="Path to NPZ file containing minimal surface data (anchors + edge_points)")
    p.add_argument("--n_points", type=int, default=256,
                   help="Number of edge points per sample (default: 256)")
    p.add_argument("--no_sort", action="store_true",
                   help="Shuffle points within each sample (remove ordering bias)")
    ...
    p.add_argument("--output_begin_trajectory", action="store_true",
                   help="Output trajectory as NPZ files")

    # Logging
    p.add_argument("--exp_name", type=str, default="minimal_surface_multimode")
...
def visualize_minimal_surface(edge_points, anchors, save_dir, name_prefix, sample_indices=None):
    """
    Visualize minimal surface samples: edge points + anchors

    Args:
        edge_points: (B, N, 2) numpy array - edge points
        anchors: (B, 3, 2) numpy array - anchors (not flattened)
        save_dir: output directory
        name_prefix: filename prefix
        sample_indices: sample indices to visualize
    """
...
def vis_begin_minimal_surface(args, dataset, path, device):
    """
    Visualize minimal surface trajectories from t=0 to t=1
    """
    num_samples = 4
    ...
    x0_np, x1_np, anchors_np = dataset.compute_batch(idx_batch=idx_batch, epoch=0, step=0)
    x0 = torch.from_numpy(x0_np).to(device)
    x1 = torch.from_numpy(x1_np).to(device)

    # anchors_np is (k, 6), reshape to (k, 3, 2) for visualization
    anchors_vis = anchors_np.reshape(-1, 3, 2)
    ...
    print(f"[VIS] Saved minimal surface visualizations to: {begin_dir}")
...
def main(args=None):
    ...
    if _is_rank0():
        ...
        print(f"[Config] mode={args.mode}, n_points={args.n_points}, p_drop={args.p_drop}")
        if args.mode in ["minibatch_ot", "eqotfm"]:
            print(f"[Config] ot_solver={args.ot_solver}, num_workers={args.num_workers}")
        if args.mode == "eqotfm":
            print(f"[Config] point_ot_solver={args.point_ot_solver}")
        if args.use_PE:
            print(f"[Config] use_PE=True (no learnable positional encoding)")
    ...
    # Uniform distribution for x0 in [-1,1]^2
    ...
    # Mode-based dataset selection
    if args.mode == "minibatch_ot":
        dataset = MinimalSurfaceMiniBatchOTDataset(
        ...
        if _is_rank0():
            ...
            print(f"[Dataset] Mode: MiniBatch OT (solver={args.ot_solver})")

    elif args.mode == "eqotfm":
        dataset = MinimalSurfaceEqOTFMDataset(
            data_path=args.data_path,
            point_ot_solver="greedy",
            batch_ot_solver="hungarian",
            num_workers=args.num_workers,
        )
        if _is_rank0():
            print(f"[Dataset] Loaded {dataset.num_meshes} samples")
            print(f"[Dataset] Mode: EqOTFM (point={args.point_ot_solver}, batch={args.ot_solver})")

    else:  # default
        dataset = MinimalSurfaceDataset(data_path=args.data_path)
        if _is_rank0():
            print(f"[Dataset] Loaded {dataset.num_meshes} samples")
            print(f"[Dataset] Mode: default (no OT)")

    # If --no_sort, shuffle points within each sample (reproducible)
    if args.no_sort and hasattr(dataset, 'edge_points'):
        if _is_rank0():
            print("[Dataset] Shuffling points within each sample (--no_sort)...")
        shuffle_seed = 0x12345678
        rng = np.random.default_rng(seed=shuffle_seed)
        num_samples = dataset.edge_points.shape[0]
        num_points = dataset.edge_points.shape[1]
        for i in range(num_samples):
            perm = rng.permutation(num_points)
            dataset.edge_points[i] = dataset.edge_points[i][perm]
        if _is_rank0():
            print(f"[Dataset] Shuffled {num_samples} samples with seed={shuffle_seed}")

    # Visualization
    if args.vis_begin and _is_rank0():
        vis_begin_minimal_surface(args, dataset, path, device)

    if _dist_available():
        dist.barrier()

    # Model selection based on --use_PE
    ...
    if _is_rank0():
        ...
        print(f"[Model] {ModelCls.__name__} with {total_params/1e6:.2f}M parameters")
    ...
    # Trainer
    trainer = MinimalSurfaceCondTrainer(
        path=path,
    ...
    if _is_rank0():
        print(f"[Trainer] MinimalSurfaceCondTrainer initialized (p_drop={args.p_drop})")
    ...
    try:
        ...
        if _is_rank0():
            ...
            ckpt_path = save_checkpoint(
                ...
                args,
                ckpt_dir,
                arch_name=f"MinimalSurface_{args.mode}_epoch{trainer.start_epoch+1:03d}",
            )
